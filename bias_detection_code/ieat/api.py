from ieat.models import SENTExtractor, OpenAIExtractor, LogitExtractor, SimCLRExtractor, ModelZooExtractor
from weat.test import Test
from ieat.utils import tests_all
...
def test_all(
	...
	for model_type, model_params in tqdm(model_types.items()):
		print(f"# {model_type} #")
		extractor = _load_model(
			model_type, *model_params, from_cache=from_cache
		)
		for test_data in tqdm(to_test):
			print(f"## {test_data.name} ##")
			if switchXY:
				categories = [
					os.path.join('data/experiments', cat) for cat in
					(test_data.Y, test_data.X, test_data.A, test_data.B)
				]
			elif switchAB:
				categories = [
					os.path.join('data/experiments', cat) for cat in
					(test_data.X, test_data.Y, test_data.B, test_data.A)
				]
			else:
				categories = [
...
def _load_model(model_type, *model_params, **model_kwargs):
	...
	elif model_type in ["byol", "npid", "odc", "moco_v1", "moco_v2", "simclr_openselfsup", "relative_loc", "rotation",
						"dc", "supervised", "random_torch_1", "random_torch_2", "random_torch_3", "jigsaw", "cluster_fit", "swav",
						'supervised_probe', 'simclr_probe', 'moco_v1_probe', 'moco_v2_probe', 'odc_probe', 'scratch_probe',
						'simclr_vissl_200', 'simclr_vissl_400', 'simclr_vissl_800', 'swav_800', 'swav_200', 'rotation_105',
						'relative_loc_probe', 'rotation_probe', 'npid_probe', 'byol_probe']:
		return ModelZooExtractor(
			model_type,
			*model_params,
			**model_kwargs
		)
