...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, width_mult=1, return_level='GAP'):
    ...
    def _forward_impl(self, x):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        if self.return_level == 'layer_0':
            return x
        x = self.layer1(x)
        if self.return_level == 'layer_1':
            return x
        x = self.layer2(x)
        if self.return_level == 'layer_2':
            return x
        x = self.layer3(x)
        if self.return_level == 'layer_3':
            return x
        x = self.layer4(x)
        if self.return_level == 'layer_4':
            return x
...
def resnet50x1(return_level='GAP', **kwargs):
    return ResNet(Bottleneck, [3, 4, 6, 3], width_mult=1, return_level=return_level)
